...
#' @export
plotEPG <- function(Data,kitname,threshT=0,refcond=NULL) {
		...
		for (peak in 1:length(bpVec)){
			if (drawpeaks) { # Create corners of peak triangle.
			 ...
			 yCords <- c(0, hVec[[1]][peak], 0)
			  ...
			  if(length(ind)>0) rect(x1,0,x2,hVec[[sn[sind]]][ind],border=tolower(as.character(colors[color])),col=sind,lwd=0.1)
			  #if(length(ind)>0) polygon(c(x1,mean(x1,x2),x2),c(0,hVec[[sn[sind]]][ind],0),border=tolower(as.character(colors[color])),col=sind,lwd=0.1)
                   }
			} 
		} #end for each peak
	}#end for each color
 ...
 #START FUNCTION
  ...
  for(loc in locs) { #impute missing heights with 1
   ...
   generateEPG(typingKit=kitname,alleleList=alist,heightList=hlist, locus=kitlocs,sampleName=sname, refList=clist,refnames=names(refcond))
}

   ...
   if(!any(is.na(kitinfo))) {
    for(loc in locs) { #for each dyes
     ...
     for(an in Avec) { 
       ...
       if(length(ind)==0) ind <- which.min(abs(as.numeric(subkit$Allele) - as.numeric(an))) #nearest neighbour (in allele name)-> maximum of the alleles
#       if(length(ind)==0) ind <- which.min(abs(as.numeric(subkit$Allele) - mean(as.numeric(subkit$Allele)))) #average of the alleles.

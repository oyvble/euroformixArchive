...
deconvolve = function(mlefit,alpha=0.95,maxlist=1000){
 ...
 Glist <- getGlist(model$popFreq) #get genotype list with genotypes and corresponding frequencies, correspond to values of GClist2
 deconvlisti <- list() #list per locus for all contributors
 for(loc in locs) {
   ii <- which(locs==loc)
   genv <-  paste0(Glist[[loc]]$G[,1],"/",Glist[[loc]]$G[,2]) #get vector of genotypes

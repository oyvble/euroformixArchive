...
contLikMLEpara = function(nC,samples,popFreq,refData=NULL,condOrder=NULL,knownRef=NULL,xi=NULL,prC=0,nDone=1,threshT=50,fst=0,lambda=0,pXi=function(x)1,delta=10,kit=NULL,verbose=TRUE,maxIter=30){
 ...
 inputlist <- list(nC=nC,samples=samples,popFreq=popFreq,refData=refData,condOrder=condOrder,knownRef=knownRef,xi=xi,prC=prC,nDone=1,threshT=threshT,fst=fst,lambda=lambda,pXi=pXi,delta=delta,kit=kit,verbose=verbose,maxIter=maxIter)
 inputlist <- rep(list(inputlist),nCl) #number of clusters

 fclust<-function(x) {
  library(euroformix)
  return(euroformix::contLikMLE(x$nC,x$samples,x$popFreq,x$refData,x$condOrder,x$knownRef,x$xi,x$prC,x$nDone,x$threshT,x$fst,x$lambda,x$pXi,x$delta,x$kit,x$verbose,x$maxIter))
 }

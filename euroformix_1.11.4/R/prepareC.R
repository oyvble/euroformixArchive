...
#       if(length(ind)==0) ind <- which.min( abs(subkit$Size - mlist[[loc]]) ) #Input with mean of the sizes
 ...
 allAbpind <- as.numeric() #vector with allele indexes used to denote what allele an allele backward-stutters to.
 for(loc in locs) { #for each marker
   ...
   for(j in 1:length(numAnames)) { #for each alleles
    bpind <- as.numeric() #must have length 0 in default
    if( is.na(numAnames[j]) ) { #if not a number (i.e. it is an NA

     #UPDATE: MUST HANDLE general LUS HERE:
     isLUS <- grepl(LUSsymbol,anames[j]) #detect whether LUS variant is used
     if( isLUS ) {
      tmp <- as.numeric(unlist(strsplit(anames[j],LUSsymbol))) #convert to numeric
      stutt <- paste0(tmp[1]-1,LUSsymbol,tmp[2]-1) #stutter-allele
      if(length(tmp)>2) stutt <- paste0(stutt,tmp[-(1:2)],collapse=LUSsymbol) #add other LUS variants
      bpind <- which(stutt==anames ) #store index of what allele it stutters to
     } #otherwise BPind[j] = 0 which is default meaning that the allele does not stutter to any allele
   ...
   allAbpind <- c(allAbpind,BPind) #vector with all base pair

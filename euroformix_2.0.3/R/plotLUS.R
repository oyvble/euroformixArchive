...
#' @param Data List of adata- and hdata-elements. adata must be separated with LUSsymbol
...
plotLUS <- function(mixData,sn="",refData=NULL,threshT=0,LUSsymbol="_") {
 ...
 for(l in 1:nL) {
  ...
  if(!is.null(refData)) {
   av2 <-  unique(c(sapply(refData,function(x) x[[which(toupper(names(x))==locs[l])]]$adata))) #loci is not casesensetive for refs
 ...
 #print(tab[l,1])
  ...
  #Add ref info:
  av <-  sapply(refData,function(x) x[[which(toupper(names(x))==tab[l,1])]]$adata) #loci is not casesensetive for refs
